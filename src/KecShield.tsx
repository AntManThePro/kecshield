...
const TWC_CATEGORIES: Record<CategoryKey, CategoryInfo> = {
  attendance: {
...
const KecShield = () => {
  ...
  const addIncident = () => {
    ...
    const incident: Incident = {
      ...
      docStatus: Object.fromEntries(
        TWC_CATEGORIES[newIncident.category].docs.map((d: string) => [d, false])
      )
    };
  ...
  const deleteIncident = (id: number) => {
    if (confirm('Delete incident? This cannot be undone.')) {
      setIncidents(incidents.filter((i: Incident) => i.id !== id));
      setExpandedIncident(null);
    }
  };

  const updateDocStatus = (incidentId: number, doc: string, status: boolean) => {
    setIncidents(incidents.map((i: Incident) =>
      i.id === incidentId
  ...
  const generateTWCReport = () => {
    const report = incidents.map((inc: Incident) => {
...
---`;
  ...
  const Dashboard = () => {
    const stats = {
      ...
      critical: incidents.filter((i: Incident) => TWC_CATEGORIES[i.category].risk === 'CRITICAL').length,
      thisWeek: incidents.filter((i: Incident) => {
        const d = new Date(i.date);
        const today = new Date();
        return (today.getTime() - d.getTime()) / (1000 * 60 * 60 * 24) <= 7;
      }).length,
      documented: incidents.filter((i: Incident) => Object.values(i.docStatus).some(Boolean)).length
    };

    const byCategory = Object.fromEntries(
      Object.entries(TWC_CATEGORIES).map(([key, val]) => [
        key,
        incidents.filter((i: Incident) => i.category === key).length
      ])
    );
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select
              ...
              onChange={(e) => setNewIncident({ ...newIncident, category: e.target.value as CategoryKey })}
        ...
        <div>
          ...
          {newIncident.evidence.length > 0 && (
            <div className="mt-3 space-y-2">
              {newIncident.evidence.map((file: EvidenceFile, i: number) => (
                <div
                  key={i}
                ...
                >
                  ...
                  <button
                    onClick={() => setNewIncident({
                      ...newIncident,
                      evidence: newIncident.evidence.filter((_: EvidenceFile, j: number) => j !== i)
                    })}
        ...
        >
          ...
          <div style={{ color: NEXUS.cyan }} className="text-xs space-y-1 mb-3">
            ...
            {category.docs.map((doc: string, i: number) => (
          ...
          <div style={{ color: NEXUS.yellow }} className="text-xs space-y-1">
            ...
            {category.examples.map((ex: string, i: number) => (
              <div key={i} className="flex items-center gap-2 ml-2">
                <span>•</span>
                <span>{ex}</span>
              </div>
  ...
  const IncidentDetailsView = () => {
    const incident = incidents.find((i: Incident) => i.id === expandedIncident);
    ...
    return (
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-sm leading-relaxed bg-opacity-30 p-4 rounded border"
            style={{ color: NEXUS.cyan, borderColor: NEXUS.cyan, backgroundColor: `${NEXUS.cyan}08` }}>
            {incident.description}
          </p>
        </div>
        ...
        {incident.evidence.length > 0 && (
          <div>
            ...
            <div className="space-y-2">
              {incident.evidence.map((file: EvidenceFile, i: number) => (
                <div
                  key={i}
